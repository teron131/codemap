/** Checks likely-entry ranking shared by summary and inspect output. */
import { describe, expect, it } from "vitest";

import type { GraphNode, GraphPayload } from "../src/codemap/source/graph/index.js";
import {
  buildLikelyEntries,
  buildPathRankedLikelyEntries,
} from "../src/codemap/source/graph/index.js";

describe("source graph likely-entry ranking", () => {
  it("prefers production entry files over test files in normal graph rankings", () => {
    ...
    expect(entries.map((entry) => entry.title)).not.toContain("src/index.test.ts");
  ...
  it("prefers core large-repo fallback entries over alphabetical extension entries", () => {
    ...
    expect(entries[0]?.description).toBe("Fallback entry candidate; detailed graph skipped.");
  ...
  it("prefers package and implementation surfaces over utility centrality", () => {
    const entries = buildLikelyEntries(
      ...
      [
        ...incomingEdges("libs/langchain/langchain_classic/_api/__init__.py", 800),
        ...outgoingEdges("libs/langchain/langchain_classic/_api/__init__.py", 2),
        ...incomingEdges("libs/core/langchain_core/exceptions.py", 72),
        ...incomingEdges("libs/core/langchain_core/language_models/chat_models.py", 39),
        ...outgoingEdges("libs/core/langchain_core/language_models/chat_models.py", 26),
        ...incomingEdges("libs/langchain_v1/langchain/agents/middleware/types.py", 55),
        ...outgoingEdges("libs/langchain_v1/langchain/agents/middleware/types.py", 7),
  ...
  it("limits package surfaces before filling likely entries with implementations", () => {
    const entries = buildLikelyEntries(
      ...
      [
        ...
        ...incomingEdges("libs/core/langchain_core/language_models/__init__.py", 70),
        ...
        ...incomingEdges("libs/core/langchain_core/language_models/chat_models.py", 45),
    ...
    expect(titles.slice(0, 5)).not.toContain("libs/langchain-classic/src/load/import_map.ts");
  });
});
