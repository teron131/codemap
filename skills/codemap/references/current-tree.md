...
Use `summary` to identify inventory, likely entries, import counts, layers, starting paths, and intent clues from README or likely-start file docstrings.
...
Inspect profiles are useful before edits because they show target-specific evidence: directory profiles, file profiles, function/class profiles, variable definitions, imports, importers, contained symbols, calls, long functions, and file-profile hints.
...
Use `signals` for structural refactor evidence. The default output is a compact bucket overview, then other sections expose the underlying measurements:
