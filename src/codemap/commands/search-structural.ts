...
export function commandSearchMatch(options: SearchMatchOptions): number {
  ...
  if (unavailableLanguages.length > 0) {
    console.error(`Unavailable syntax languages: ${unavailableLanguages.join(", ")}.`);
  }
...
export function commandSearchCalls(options: SearchCallsOptions): number {
  ...
  if (matches.length === 0 && !options.json) {
    ...
    console.log(`... ${matches.length - visibleMatches.length} more call sites`);
  }
  return matches.length > 0 ? 0 : 1;
}
...
function printSyntaxMatches(matches: SyntaxMatch[], { jsonOutput }: { jsonOutput: boolean }): void {
  if (jsonOutput) {
  ...
  for (const match of matches) {
    ...
    console.log(`${match.filePath}:${match.line}:${match.column}${engine}: ${text}`);
  }
}
