...
export function docstringPreview(
	docstring: string | null,
	{
		fallback = "none",
		maxLength = 120,
	}: { fallback?: string; maxLength?: number } = {},
): string {
	if (!docstring) {
		return fallback;
	}
...
export function resolveFocusPathsInOrder(
	...
	return focusFiles.map((rawFocusPath) =>
		path.isAbsolute(rawFocusPath)
			? rawFocusPath
			: path.resolve(moduleRoot, rawFocusPath),
	);
}
...
export function functionPriority(
	report: FunctionReport,
): [number, number, number] {
	const loweredName = report.name.toLowerCase();
	const hasDocstring = report.docstring ? 0 : 1;
	const isDunder =
		loweredName.startsWith("__") && loweredName.endsWith("__") ? 1 : 0;
	let publicRank = 2;
	if (LIKELY_MAIN_FUNCTION_NAMES.includes(loweredName as never)) {
		publicRank = 0;
	} else if (
		LIKELY_MAIN_FUNCTION_PREFIXES.some((prefix) =>
			loweredName.startsWith(prefix),
		)
	) {
		publicRank = 1;
...
export function functionSignalCandidates(
	...
	const qualifiedName =
		ownerName === null ? report.name : `${ownerName}.${report.name}`;
...
export function collectSignalFunctions(
	report: FileReport,
): Array<[string, FunctionReport]> {
...
export function countFunctions(functions: FunctionReport[]): number {
	return functions.reduce(
		(total, report) => total + 1 + countFunctions(report.nestedFunctions),
		0,
	);
}
...
export function countClassMethods(classes: ClassReport[]): number {
	return classes.reduce(
		(total, classReport) =>
			total +
			countFunctions(classReport.methods) +
			countClassMethods(classReport.nestedClasses),
		0,
	);
}
...
export function supportedFocusPaths(paths: string[]): string[] {
	return paths.filter(
		(filePath) =>
			isFile(filePath) && DOCSTRING_SUFFIXES.has(path.extname(filePath)),
	);
}
...
export function collectReports(
	...
	const supportedFiles =
		focusFiles !== null && focusFiles.length > 0
			? supportedFocusPaths(
					resolveFocusPathsInOrder(focusFiles, { moduleRoot }),
				)
	...
	for (const filePath of supportedFiles) {
		...
		if (PYTHON_SUFFIXES.has(path.extname(filePath))) {
			...
			reports.push(
				buildTypescriptFileReport(filePath, { displayPath: shownPath }),
			);
		}
	}
...
export function buildDocstringsData(targetPath: string): DocstringsData {
	...
	return {
		...
		functions: reports.reduce(
			(total, report) => total + countFunctions(report.functions),
			0,
		),
		class_methods: reports.reduce(
			(total, report) => total + countClassMethods(report.classes),
			0,
		),
		classes: reports.reduce(
			(total, report) => total + countClasses(report.classes),
			0,
		),
		file_reports: reports.map((report) => ({
			...
			functions: report.functions.map((functionReport) =>
				functionToDict(functionReport),
			),
...
export function buildFilePreviews(
	...
	const shownReports =
		maxFiles <= 0 ? focusedReports : focusedReports.slice(0, maxFiles);
...
export function buildDocstringSignals(
	...
	const fileDocstringCount = focusedReports.filter(
		(report) => report.fileDocstring,
	).length;
	const shownReports =
		maxFiles <= 0 ? focusedReports : focusedReports.slice(0, maxFiles);
	...
	for (const report of shownReports) {
		for (const [qualifiedName, functionReport] of collectSignalFunctions(
			report,
		)) {
