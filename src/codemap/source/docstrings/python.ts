...
export function formatSignature(rawArguments: string, rawReturns: string | null): [string, string] {
...
export function buildPythonFileReport(
  ...
  for (const definition of definitions) {
    ...
    if (definition.kind === "function") {
      report.functions.push(buildFunctionReport(definition, definitions, lines));
    } else {
...
function childDefinitions(definition: Definition, definitions: Definition[]): Definition[] {
...
function parentNames(definitions: Array<Omit<Definition, "parents">>, index: number): string[] {
...
function definitionDocstring(definition: Definition, lines: string[]): string | null {
  let lineIndex = definition.bodyStart;
  while (lineIndex < definition.bodyEnd && !(lines[lineIndex] ?? "").trim()) {
    lineIndex += 1;
  }
  if (lineIndex >= definition.bodyEnd || indentOf(lines[lineIndex] ?? "") <= definition.indent) {
    return null;
  }
  return readDocstringAt(lines, lineIndex);
}
...
function cleanDocstring(value: string): string {
  ...
  const indents = lines.filter((line) => line.trim()).map((line) => indentOf(line));
...
function pythonParseError(source: string): string | null {
  for (const line of source.split(/\r?\n/)) {
    ...
    if (
      ...
      !/^async\s+def\s+[A-Za-z_][A-Za-z0-9_]*\s*\(.*\)\s*(?:->\s*.*?)?:/.test(stripped) &&
