...
export function pythonFunctionIdentifier(
	relPath: string,
	parents: string[],
	name: string,
): string {
...
export function collectPythonImportsAndVariables(
	...
	for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
		...
		if (fromMatch) {
			...
			const targetName =
				level > 0 ? `${".".repeat(level)}${moduleName}` : moduleName;
		...
		if (assignment.target.trim() === "__all__") {
			metrics.exportedNames.push(
				...literalStrings(
					multilinePythonValue(lines, lineIndex, assignment.value),
				),
			);
		}
	}
...
function updateTripleStringState(
	inTripleString: boolean,
	line: string,
): boolean {
	let state = inTripleString;
	for (const delimiter of ['"""', "'''"]) {
		const matches = line.matchAll(new RegExp(delimiter, "g"));
