...
import { buildLikelyEntries, buildPathRankedLikelyEntries } from "./likely-entries.js";
...
export function buildInventoryView(
  ...
  for (const node of fileNodes) {
    const rootName = node.filePath.includes("/") ? (node.filePath.split("/", 1)[0] ?? ".") : ".";
...
export function buildIntentView(
  ...
  return {
    ...
    filePreviews: previews.filter((item) => isUsefulIntentPreview(item.preview)),
  };
}
...
export function relationshipCountsFromGraph(
  ...
  for (const edge of edges) {
    ...
    if (tags.has("typescript") || tags.has("tsx") || tags.has("javascript") || tags.has("jsx")) {
...
function appendHotspots(lines: string[], value: unknown): void {
  ...
  const hotspots = allRows
    ...
    .map((item) => {
      ...
      const facts = [fanIn !== null ? `fan-in ${fanIn}` : null, location].filter(
        (fact) => fact !== null,
      );
  ...
  lines.push("");
  lines.push(`## Hotspots${genericCount > 0 ? ` (hidden generic: ${genericCount})` : ""}`);
  lines.push(...hotspots);
}
...
function appendClusters(lines: string[], value: unknown): void {
  ...
  const clusters = records
    .map((item) => {
      ...
      const label = rawLabel !== null && labelCounts.get(rawLabel) === 1 ? rawLabel : null;
      ...
      const topNodes = uniqueStrings(
        arrayValue(row.top_nodes)
          ...
          .filter((node): node is string => node !== null && !genericBackendName(node)),
      ).slice(0, 4);
      const facts = [
        members !== null ? `${members} nodes` : null,
        cohesion !== null ? `cohesion=${formatScore(cohesion)}` : null,
        label !== null && topNodes.length > 0 ? `top: ${topNodes.join(", ")}` : null,
      ].filter((fact) => fact !== null);
      const title = label ?? topNodes.join(", ");
      return title ? `- ${title}${facts.length > 0 ? ` (${facts.join("; ")})` : ""}` : null;
    })
    .filter((item) => item !== null);
...
function appendSparseSymbolNote(lines: string[], nodeLabels: unknown): void {
  ...
  const symbolCount =
    (labels.get("Function") ?? 0) + (labels.get("Class") ?? 0) + (labels.get("Method") ?? 0);
  if (symbolCount > 0) {
    return;
  }
  lines.push("note: no function/class/method nodes; summary is file-level only.");
}
...
function qualifiedNameSuffix(qualifiedName: string | null, name: string): string | null {
