...
export function renderSignalText(payload: Record<string, unknown>, section: string): string {
  ...
  if (section === "top") {
    ...
    const functionMetrics = arrayValue<Row>(payload.functionMetrics);
  ...
  if ("files" in payload) {
    appendFiles(lines, arrayValue<Row>(payload.files));
  }
  return `${lines.join("\n")}\n`;
}
...
function appendTop(lines: string[], top: Record<string, unknown>): void {
  const functionMetrics = arrayValue<Row>(top.functionMetrics);
  const functionsByMentions = arrayValue<Row>(top.functionsByMentions);
  const variablesByNameLength = arrayValue<Row>(top.variablesByNameLength);
  if (
...
function appendRelationships(lines: string[], relationships: Record<string, unknown>): void {
  ...
  appendFileCountRows(
    ...
    arrayValue<Row>(relationships.top_local_import_hubs),
  );
  appendFileCountRows(
    lines,
    "Top Inheritance Hubs",
    arrayValue<Row>(relationships.top_inheritance_hubs),
  );
  lines.push("");
}
...
function appendVariableSignals(lines: string[], payload: Record<string, unknown>): void {
  appendVariableNameLengthRows(lines, arrayValue<Row>(payload.byNameLength));
  appendDefinitionRows(
    lines,
...
function appendLengths(lines: string[], lengths: Record<string, unknown>): void {
  ...
  for (const [key, label] of [
    ...
    const items = arrayValue<Row>(section.items);
    if (items.length === 0) {
      continue;
    }
...
function appendDocstringSignals(lines: string[], payload: Record<string, unknown>): void {
  ...
  appendDocstringPreviewRows(
    ...
    arrayValue<Row>(payload.file_docstring_previews),
  );
  appendDocstringPreviewRows(
    lines,
    "Likely Main Function Docstrings",
    arrayValue<Row>(payload.likely_main_function_docstrings),
    { includeLine: true },
  );
  lines.push("");
}
...
function appendDocstrings(lines: string[], payload: Record<string, unknown>): void {
  ...
  for (const report of reports) {
    ...
    appendDefinitionPreviewChildren(lines, arrayValue<Row>(report.functions), {
      prefix: "function",
    });
    appendDefinitionPreviewChildren(lines, arrayValue<Row>(report.classes), {
      prefix: "class",
    });
  }
  lines.push("");
...
function appendDefinitionPreviewChildren(
  ...
  for (const row of rows) {
    ...
    if (prefix === "class") {
      appendDefinitionPreviewChildren(lines, arrayValue<Row>(row.methods), {
        prefix: "method",
      });
      appendDefinitionPreviewChildren(lines, arrayValue<Row>(row.nested_classes), {
        prefix: "class",
      });
    } else {
      appendDefinitionPreviewChildren(lines, arrayValue<Row>(row.nested_functions), {
        prefix,
      });
    }
  }
