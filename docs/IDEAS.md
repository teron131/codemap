...
- Keep direct paths and exact symbol definitions current-tree-first. For ordinary source search, prioritize Codebase Memory code search and use current-tree concept, text, and partial-term evidence when the backend is unavailable or has no usable answer. Keep relationship and vocabulary-bridging work in the explicit `--graph` and `--semantic` lanes.
- When a multi-term phrase has no useful whole-query source answer, prefer supported implementation candidates, then rank them by distinct normalized term coverage at the file level before ordinary source usefulness. Rank the available matches from each bounded ripgrep collection rather than its first few traversal-order rows, and state when the collection bound makes that evidence partial.
